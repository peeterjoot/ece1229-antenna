...
&\quad + \lr{ \rho_\txtm \bcH + \epsilon_0 \bcE \cross \bcM }
=
...
%}
%
On the LHS we have the rate of change of momentum density, the electric Lorentz force density terms, the dual (magnetic) Lorentz force density terms, and on the RHS the \textAndIndex{momentum flux} terms.
%
\paragraph{In the frequency domain.}
\index{frequency domain}
%
With frequency domain fields \( \bcE = \Real \BE e^{j \omega t}\) and \(\bcH = \Real \BH e^{j \omega t} \).  Using the electric field dot product as an example, note that we can write
%
...
\end{equation}
%
Permuting indexes and taking the difference kills all the time dependent terms, even without averaging
%
