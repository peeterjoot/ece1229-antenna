%
% Copyright © 2015 Peeter Joot.  All Rights Reserved.
% Licenced as described in the file LICENSE under the root directory of this GIT repository.
%
\input{../assignment.tex}
\renewcommand{\basename}{advancedantennaProblemSet3}
\renewcommand{\dirname}{notes/ece1229/}
\newcommand{\keywords}{Advanced Antenna Theory, ECE1229H}
\newcommand{\dateintitle}{}
\input{../peeter_prologue_print2.tex}

\usepackage{peeters_layout_exercise}
%\usepackage{macros_mathematica}
\usepackage{ece1229}
\usepackage{siunitx}
\usepackage{esint} % \oiint

\renewcommand{\QuestionNB}{\alph{Question}.\ }
\renewcommand{\theQuestion}{\alph{Question}}
%
%\makeatletter
%\lst@AddToHook{Init}{\mathligsoff}
%\makeatother
%
%\let\origlstlisting=\lstlisting
%\let\endoriglstlisting=\endlstlisting
%\renewenvironment{lstlisting}
%    {\mathcode`\-=\hyphenmathcode
%     \everymath{}\mathsurround=0pt\origlstlisting}
%    {\endoriglstlisting}

\beginArtNoToc
\generatetitle{ECE1229H Advanced Antenna Theory.  Problem Set 3: Dipoles and corner cube antennas}
%\chapter{Dipoles and corner cube antennas}
\index{corner cube}
\index{reflect antenna}
\label{chap:advancedantennaProblemSet3}

%\section{Disclaimer}
%
%This is an ungraded set of answers to the problems posed.

\input{advancedantennaProblemSet3Problem1.tex}
\input{advancedantennaProblemSet3Problem2.tex}
\input{advancedantennaProblemSet3Problem3.tex}

\section{Mathematica Sources}

The Mathematica code associated with these notes is available under
\href{https://github.com/peeterjoot/mathematica/tree/master/ece1229/ps3}{ece1229/ps3/}
within the github repository:

git@github.com:peeterjoot/mathematica.git

The notebooks for this problem set are

\input{ps3mathematica.tex}

%The data and figures referenced in these notes were generated with versions not greater than:
%
%FIXME:
%\begin{itemize}
%\item commit 5641358d1f397dab8a4053f7fb9681b0d532cad6
%\end{itemize}

\EndArticle
